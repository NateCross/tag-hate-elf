# 100% Implementation Defense

## Datasets

- [Reddit PH Hate Speech](./datasets/annotated-dataset.csv)
- [2016 and 2022 PH Elections Hate Speech](./datasets/2016_2022_hate_speech_filipino.csv)
- [Final Dataset](./datasets/datasetall.csv)
  - [Train Dataset](./datasets/dataset-train.csv)
  - [Test Dataset](./datasets/dataset-test.csv)

## 50%

- [Reddit Scrape](./notebook/reddit-scrape.ipynb)
- [Filter Data](./notebook/filter-data.ipynb)
- [Processing After Annotation](./notebook/final-prep.ipynb)

## Models

- [Bernoulli Naive Bayes](./bayes_train.ipynb)
- [LSTM](./lstm_train.ipynb)
- [mBERT](./bert_train.ipynb)
- [Ensembles](./ensembles.ipynb)
  - [Logistic Regression](./lr_train.ipynb)

## Appendix

[Paper](https://drive.google.com/file/d/1bLW5E5JxVDI5O1lUbKNaj-UxmfVaFoL5/view?usp=drive_link)